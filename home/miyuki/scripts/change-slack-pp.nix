{ pkgs, inputs, config, ... }: {
  sops = let secretsPath = builtins.toString inputs.secrets;
  in {
    secrets = {
      "slack/profile-picture-setter".sopsFile =
        "${secretsPath}/hosts/miyuki.yaml";
    };
  };
  ...
  home.packages = with pkgs;
    [
      (writeShellApplication {
        ...
        runtimeInputs = [ bc imagemagick openssl curl ];
        ...
        text = ''
          ...
          function upload_pp {
            local token="$(<${
              config.sops.secrets."slack/profile-picture-setter".path
            })"
            local filename="$1"
