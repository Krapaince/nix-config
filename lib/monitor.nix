...
let
  resolveMonitors = (
    monitors:
    let
      unresolvedMonitors = lib.filter (m: (builtins.isNull m.x) && (builtins.isNull m.y)) monitors;
      resolvedMonitors = lib.filter (m: (lib.isInt m.x) && (lib.isInt m.y)) monitors;
  ...
  resolveMonitorsPosition = lib.lists.foldl (
    ...
    let
      ...
      monitor = lib.optionals canResolve [
        (lib.attrsets.removeAttrs (computeRelativePosition unresolvedMonitor resolvedRelativeMonitor) [
          "direction"
          "relativeTo"
        ])
      ];
  ...
  findResolvedRelativeMonitor = (
    ...
    lib.lists.findFirst (
      m: (m.name == unresolvedMonitor.relativeTo) && (m ? x) && (m ? y)
    ) null monitors
  );
  ...
  computeRelativePosition = (
    ...
    let
      directionToMultiplier = (
        direction: {
          v =
            if lib.strings.hasInfix "north" direction then
              -1
              ...
              0;
          h =
            if lib.strings.hasInfix "west" direction then
              -1
