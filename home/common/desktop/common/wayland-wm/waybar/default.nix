...
}:
{
  ...
  programs.waybar = {
    ...
    settings =
      ...
      in
      map (bar: lib.trivial.mergeAttrs common_modules bar) [
        (
          let
        ...
        (
          ...
          in
          {
            ...
            modules-right =
              (if (network-interfaces.wired.name != null) then [ "network#net-wired" ] else [ ])
              ++ (if (network-interfaces.wireless.name != null) then [ "network#net-wireless" ] else [ ])
              ++ [
                "temperature#cpu"
                "cpu"
                "memory"
